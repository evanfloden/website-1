...
The folder where the `codecheck.yml` file is stored is call the _CODE CHECK bundle_.
It is the folder that also includes a directory `codecheck` (or `.codecheck`) for all files created during a check by the codechecker, see [CODE CHECK bundle documentation]().

# Content

## Explicit document and directive

The file MUST include three dashes (`---`), the directives end marker line, to seperate the directive from document content.
The file SHOULD define the YAML version in the directive.

While supports [bare documents](https://yaml.org/spec/1.2/spec.html#id2800132), an explicit indication of the format is preferable for the CODE CHECK use case.
A `codecheck.yml` file is therefore either an _explicit document_ or a _directives document_.
...
YAML allows concatenation of documents in form of a [YAML _stream_](), which is used to separate the content contributed by the authors and codecheckers.
...
- `notes`: Short summary of the check report.
